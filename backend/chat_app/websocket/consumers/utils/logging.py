...
class ChatConsumerLogging:
    ...
    def log_connect_done(user, session_id):
        logger.info(f"{lu.CC_MAIN} All setup steps for {BOLD}{user}{UNBOLD} succeeded. ChatSession ID: {BOLD}{session_id}{RESET}")

    @staticmethod
    def log_disconnect(user, code):
        logger.info(f"{lu.G_LINE_1}{lu.CC_MAIN} {BOLD}{user}{UNBOLD} disconnected (code: {BOLD}{code}{UNBOLD}) {RESET}{lu.G_LINE_2}")
...
class ChatListenerLogging:
    ...
    def log_connect(username, session_owner, session_id):
        logger.info(f"{lu.Y_LINE_1}{lu.CL_MAIN} {BOLD}{username}{UNBOLD} listening to: {BOLD}{session_owner}{UNBOLD}. ChatSession ID: {BOLD}{session_id} {RESET}{lu.Y_LINE_2}")
